...
public class ComSci extends HitBox{
	...
	public void render(GameContainer gc, Graphics g) {
		...
		if (!falling)
			...
			if (view == 0)
				g.drawImage(kJumpR.getSubImage(0, 6), x, y);  // gets the image where a free falling image is seen
			else 
				g.drawImage(kJumpL.getSubImage(0, 6), x, y); // gets the image where a free falling image is seen
	}
		...
		if (gc.isPaused())
	    	...
	    	if (player == 1){
	    		...
	    		if (!hit && !attacking){
	    			// FIXME heavy attacks causes a dual proc of knock back animation    	
	    			//light punch
	    			if (in.isKeyPressed(Input.KEY_NUMPAD4)) {
	    				attacking = true;
	    				if (view == 0) // if facing left
	    					kAnimation = kAnimationLPR;
	    				else           // if facing right
	...
	public void logic(){
		...
		if (y == 110 && jumping){   // when the player reaches the peak of his jump 
